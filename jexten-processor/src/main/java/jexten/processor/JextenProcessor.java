 ...
 */
@SupportedAnnotationTypes({ "jexten.Extension", "jexten.ExtensionPoint" })
...
public class JextenProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        if (!errors.hasMessages()) {
            writeOutputFile(serviceImplementations);
        }
    ...
    private void validateModuleExtension(
        ...
        if (!nonDeclared.isEmpty()) {
            ...
            if (implementations.isEmpty() && nonDeclared.size() == 1) {
                fix.append(extensions.get(0)).append(";\n\n");
            } else {
    ...
    private void writeOutputFile(Map<String, List<String>> serviceImplementations) {
